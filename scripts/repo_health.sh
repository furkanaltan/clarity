...
if printf '%s\n' "$staged" | grep -Ev '^$|^(ARCHITECTURE\.md|PROJECT_MAP\.md|README\.md|RUNBOOK\.md|docs/PROJECT_RULES\.md|scripts/repo_health\.sh|report_templates/rove_web_report\.html|rove_log_safety\.py|test_chart_engine_v2\.py|test_privacy_logs\.py)$' | grep -q .; then
  warn 'unexpected staged file present'
fi
if printf '%s\n' "$untracked" | grep -Ev '^$|^(CLAUDE\.md|docs/PROJECT_RULES\.md|scripts/repo_health\.sh|scripts/build_frontend_release\.py|rove_log_safety\.py|test_frontend_build\.py|test_chart_engine_v2\.py|test_score_v2\.py|test_mentor_v2\.py|test_coach_v3\.py|test_frontend_transaction_xss\.py|test_privacy_logs\.py)$' | grep -q .; then
