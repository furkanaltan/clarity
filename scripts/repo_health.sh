...
if printf '%s\n' "$untracked" | grep -Ev '^$|^(CLAUDE\.md|docs/PROJECT_RULES\.md|scripts/repo_health\.sh|scripts/build_frontend_release\.py|rove_log_safety\.py|test_frontend_build\.py|test_chart_engine_v2\.py|test_score_v2\.py|test_mentor_v2\.py|test_frontend_transaction_xss\.py|test_privacy_logs\.py)$' | grep -q .; then
