...
if printf '%s\n' "$staged" | grep -Ev '^$|^(ARCHITECTURE\.md|PROJECT_MAP\.md|README\.md|RUNBOOK\.md|docs/PROJECT_RULES\.md|scripts/repo_health\.sh|report_templates/rove_web_report\.html|test_chart_engine_v2\.py)$' | grep -q .; then
  warn 'unexpected staged file present'
fi
if printf '%s\n' "$untracked" | grep -Ev '^$|^(CLAUDE\.md|docs/PROJECT_RULES\.md|scripts/repo_health\.sh|scripts/build_frontend_release\.py|test_frontend_build\.py|test_chart_engine_v2\.py|test_score_v2\.py|test_mentor_v2\.py|test_frontend_transaction_xss\.py)$' | grep -q .; then
