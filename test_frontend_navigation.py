...
class FrontendNavigationTests(unittest.TestCase):
    ...
    def test_home_chart_uses_truthful_paths_and_visible_context_without_changing_data(self):
        ...
        self.assertIn('return {pts:last>=0 ? [source[last]] : []', self.frontend)
        self.assertIn('const neutralDay=range==="1T"&&!rangeData.intraday;', self.frontend)
        self.assertIn('class="chart-day-reference"', self.frontend)
