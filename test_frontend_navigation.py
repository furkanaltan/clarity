...
class FrontendNavigationTests(unittest.TestCase):
    ...
    def test_home_chart_uses_truthful_paths_and_visible_context_without_changing_data(self):
        ...
        self.assertIn('if(!win.length) win=[h[h.length-1]];', self.frontend)
        ...
        self.assertIn('function ensureOneDaySeries(range)', self.frontend)
        self.assertIn('DATA.series["1T"]=last>=0 ? [source[last]] : []', self.frontend)
        self.assertIn('drawChart("1T")', self.frontend)
        self.assertIn('ensureOneDaySeries(range);\n  const pts = DATA.series[range]', self.frontend)
        self.assertIn('dEl.textContent = pts.length===1 ? "—"', self.frontend)
